...
class User(db.Model,UserMixin):
  ...
  password_hash: so.Mapped[Optional[str]] = so.mapped_column(sa.String(256),nullable=True, default="")
  about_me : so.Mapped[str] = so.mapped_column(sa.String(140),nullable=False,default="")
  last_seen : so.Mapped[Optional[datetime]] = so.mapped_column(default= lambda: datetime.now(timezone.utc),nullable=False)
  ...
  def following_posts(self):
    # include posts from users this user follows plus the user's own posts
    followed_subq = sa.select(followers.c.followed_id).where(followers.c.follower_id == self.id)
    return (
      sa.select(Post)
      .where(sa.or_(Post.user_id == self.id, Post.user_id.in_(followed_subq)))
